        ...
        s = "layui-this", o = "layui-hide", c = "layui-disabled", u = function () {
            this.config = {
                verify: {
                    ...
                    num_chemical: [
                        /^$|^\d{1,3}(\.\d{1,4})?$/, '数字类型，支持4位小数，校验填入数值范围为0~100'
                    ],
                    num_length_float2: [
                        /^$|^\d+(\.\d{1,2})?$/, '数字类型，支持2位小数'
                    ],
                    ...
                    length_n_max_2: function (value, ele) {
                        var reg = /^\d{0,2}$/;
                        if (!reg.test(value.trim())) {
                            var notice = $(ele).data('notice') ? $(ele).data(
                                'notice') : '';
                            return notice + '允许两位正整数';
                        }
                    },
                    length_s_max_30: function (value, ele) {
                        var reg = /^\S{0,30}$/;
                        if (value && !reg.test(value.trim())) {
                            var notice = $(ele).data('notice') ? $(ele).data(
                                'notice') : ''
                            return notice + '输入内容不能超过30个字';
                        }
                    },
                    length_s_max_100: function (value, ele) {
                        var reg = /^\S{0,100}$/;
                        if (value && !reg.test(value.trim())) {
                            var notice = $(ele).data('notice') ? $(ele).data(
                                'notice') : ''
                            return notice + '输入内容不能超过100个字';
                        }
                    },
                    length_s_max_200: function (value, ele) {
                        if (value && value.trim().length > 200) {
                            var notice = $(ele).data('notice') ? $(ele).data(
                                'notice') : ''
                            return notice + '输入内容不能超过200个字';
                        }
                    },
                    length_f_max_2: function (value, ele) {
                        var reg =
                            /^(([1-9][0-9]{0,6})|(([0]\.\d{1,5}|[1-9][0-9]{0,6}\.\d{1,5})))$|(^\d{0,1}$)/
                        if (!reg.test(value.trim())) {
                            var notice = $(ele).data('notice') ? $(ele).data(
                                'notice') : '';
                            return notice + '允许输入整数位七位，小数位2位';
                        }
                    },
        ...
        if (layui.each(d, function (e, l) {
            ...
            if (r.removeClass(o), layui.each(c, function (e, t) {
                ...
                if (a[t]) {
                    ...
                    }(), {tips: [1, '#ff4c4c']}) : "alert" === u ? i.alert(f, {
                        title: "提示",
                        shadeClose: !0
